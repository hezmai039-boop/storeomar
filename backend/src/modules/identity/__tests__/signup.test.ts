...
test("an ASCII name becomes a clean lowercase-hyphen slug", () => {
  assert.equal(deriveOrgSlug("Maysoor Store"), "maysoor-store");
...
test("an Arabic name derives to the empty string — expected, not an error", () => {
  ...
  assert.equal(deriveOrgSlug("شركة ميسور للتقنية"), "");
});
...
test("a free slug is used as-is", async () => {
  const slug = await resolveUniqueOrgSlug("Maysoor Store", async () => false);
  assert.equal(slug, "maysoor-store");
});

test("a taken slug collides forward with a counter", async () => {
  const taken = new Set(["maysoor-store", "maysoor-store-2"]);
  const slug = await resolveUniqueOrgSlug("Maysoor Store", async (s) => taken.has(s));
  assert.equal(slug, "maysoor-store-3");
});

test("after the counter is exhausted it falls back to a random suffix, never loops forever", async () => {
  const taken = new Set(["maysoor-store", "maysoor-store-2", "maysoor-store-3", "maysoor-store-4", "maysoor-store-5"]);
  const slug = await resolveUniqueOrgSlug("Maysoor Store", async (s) => taken.has(s), () => "beefcafe");
  assert.equal(slug, "maysoor-store-beefcafe");
});

...
هذا الفصل جزء من وثيقة الاستراتيجية التنافسية لمنصة ميسور. يغطي مسارين
...
ميسور مبني من الأساس (انظر [02-architecture.md §1](02-architecture.md#1-النمط-المعماري-modular-monolith-قابل-للتفكك))
...
شخصية وترسل رسائل تجارية — وهي أنظمة قابلة للتطبيق فعليًا على ميسور إذا
...
أهم ما يمسّ ميسور مباشرة كمنصة تخزّن محادثات عملاء وقواعد معرفة:

| المتطلب | الأثر على ميسور |
|---|---|
| التسجيل كـ"جهة تحكم" (Controller) عبر منصة الحوكمة الوطنية للبيانات | كل مؤسسة (Organization) عميلة تحتاج مساعدتها على استيفاء هذا كجزء من التأهيل (Onboarding)، لا مسؤولية ميسور بالنيابة عنها فقط |
...
ميسور ليست ولا يجب أن تصبح محرك فوترة ضريبية. لكن الأثر غير المباشر مهم
...
- **القاعدة التصميمية لميسور:** أي فاتورة أو إيصال يظهر للعميل النهائي
  ...
  تُقر بامتثالها لمتطلبات فاتورة)، لا نصًا يُنشئه ميسور بنفسه. توليد
  "فاتورة موازية" داخل ميسور يخلق خطر تعارض مع السجل الرسمي المعتمد لدى
...
**التمييز المهم لميسور:** هذه الضوابط منصوصة تحديدًا للرسائل النصية
...
لكل بث تسويقي عبر أي قناة في ميسور — لا فقط SMS — قرارًا احترازيًا سليمًا
...
ميسور يملك أصلاً محوّل واتساب (`WhatsAppAdapter`) حسب
...
- **الشارة الزرقاء (التوثيق الرسمي، سابقًا "العلامة الخضراء"):** لا تُمنح
  ...
  تستغرق أسابيع. **الفرصة لميسور:** تقديم "مسار توثيق مُدار" كميزة مدفوعة
  للعملاء المؤسسيين — ميسور يهيئ المتطلبات (نصوص، إثباتات) ويتابع الطلب
...
- **سلّة تُشغِّل متجر تطبيقات (App Store) وزد تُشغِّل سوق تطبيقات مشابه
  ...
  للجميع وتطبيقات خاصة لعميل واحد. إدراج ميسور كتطبيق عام معتمد في كلا
  المتجرين يحوّل التكامل من "عمل تنفيذي لكل عميل" إلى **قناة توزيع
  واكتساب عملاء** — أي تاجر سلّة/زد يستطيع تثبيت ميسور بضغطة دون مبيعات
...
- كلا المنصتين تدمج أصلاً بوابات دفع محلية (مدى، STC Pay) ومنصات تقسيط
  (تابي، تمارا) وتقر بالامتثال لمتطلبات فاتورة — ما يعزز مبدأ §2.2: ميسور
...
### 3.4 مدى (mada) وSTC Pay — موقف ميسور كطرف غير معالج للدفع
...
**القرار الاستراتيجي الصحيح لميسور: عدم التحول لمزوّد خدمات دفع.**
معالجة المدفوعات المباشرة تُدخل ميسور في نطاق ترخيص ساما لمزوّدي خدمات
...
ميسور يعرض داخل محادثة واتساب **رابط دفع جاهزًا** صادرًا من نظام مرخّص
...
**الاستخدام الواقعي المناسب لميسور ليس التحقق من كل عميل يراسل واتساب**
...
الواقعي الوحيد لميسور هنا هو على مستوى **الوكيل الذكي**: التعرف على أن
...
| 5 — التوزيع عبر متاجر التطبيقات | إدراج ميسور كتطبيق عام معتمد في متجر تطبيقات سلّة وسوق تطبيقات زد | يحوّل التكامل القائم أصلاً إلى قناة اكتساب عملاء بأقل تكلفة تسويقية |
...
# الجزء الثاني: نسخة ميسور لقطاع المطاعم
...
دون أن تتحول عمليًا لمنتج مختلف. ميسور يملك بالفعل الأساس البنيوي المناسب
...
| Inventory (أساسي) | Store Integrations (قراءة فقط من Foodics) | ميسور لا يدير المخزون، يقرأ إشارة "نفد" لتعطيل عنصر في محادثة الطلب فورًا |
...
يُنشأ داخل ميسور عبر محادثة عميل (§13 أدناه) **يُدفَع إلى Foodics كمصدر
الحقيقة الوحيد للفوترة والتحضير والامتثال الضريبي** — Foodics تُقر
بامتثالها لمتطلبات فاتورة، فلا داعي لأي منطق فوترة موازٍ داخل ميسور
...
ميسور لا يعيد بناء إدارة مخزون كاملة (Foodics تدير هذا فعليًا حتى مستوى
المكوّن الواحد ضمن الوصفة). دور ميسور هنا محدد وحرج: **استهلاك إشارة
...
التوطين العميق (الجزء الأول) يرفع تكلفة تقليد ميسور من قِبل أدوات أفقية
...
ميسور من "أداة أتمتة ردود" إلى **منصة تشغيل قطاعية متعددة الفروع** يصعب
